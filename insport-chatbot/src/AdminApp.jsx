...
import { LayoutDashboard, AlertTriangle, Settings, LogOut, Activity, Image, Cpu, CheckCircle2, BarChart3, TrendingUp, MessageCircle } from 'lucide-react';
        ...
        <div className="admin-content">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px', marginBottom: '24px' }}>
                <div className="admin-panel" style={{ minHeight: '400px' }}>
                    <div className="admin-panel-title">
                        <BarChart3 size={20} color="#00f2fe" /> Vision 판독 통계 (D-Report)
                    </div>
                        ...
                        <Bar 
                            data={{
                                ...
                                datasets: [{
                                    ...
                                    backgroundColor: ['rgba(239, 68, 68, 0.7)', 'rgba(245, 158, 11, 0.7)', 'rgba(59, 130, 246, 0.7)', 'rgba(16, 185, 129, 0.7)', 'rgba(0, 242, 254, 0.7)'],
                                    borderRadius: 8
                                }]
                            }}
                            options={{
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {alerts.filter(a => a.severity === 'major').slice(0, 3).map(alert => (
                            <div key={alert.id} style={{ background: 'rgba(239,68,68,0.05)', border: '1px solid rgba(239,68,68,0.15)', padding: '16px', borderRadius: '12px' }}>
                                ...
                                <div style={{ fontSize: '0.75rem', color: '#94a3b8', marginTop: '4px' }}>"{alert.summary}"</div>
                            </div>
                        ))}
          ...
          {activeMenu === 'support' && (
            <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '24px', height: 'calc(100vh - 160px)' }}>
                ...
                <div className="admin-panel" style={{ padding: '0', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ padding: '20px', borderBottom: '1px solid #1e293b', fontWeight: 700 }}>활성 상담 세션 ({sessions.length})</div>
                    <div style={{ flex: 1, overflowY: 'auto' }}>
                        {sessions.map(s => (
                            <div 
                                key={s.id} 
                                onClick={() => selectSession(s.id)}
                                style={{ padding: '16px 20px', borderBottom: '1px solid #1e293b', cursor: 'pointer', background: selectedSessionId === s.id ? 'rgba(0, 242, 254, 0.05)' : 'transparent', borderLeft: selectedSessionId === s.id ? '4px solid #00f2fe' : '4px solid transparent' }}
                            >
                    ...
                    {selectedSessionId ? (
                        <>
                            <div style={{ padding: '16px 24px', borderBottom: '1px solid #1e293b', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <button 
                                    onClick={toggleModel}
                                    style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid ' + (isManual ? '#00f2fe' : '#ef4444'), color: isManual ? '#00f2fe' : '#ef4444', background: 'transparent', cursor: 'pointer', fontWeight: 700 }}
                                >
                            ...
                            <div style={{ flex: 1, padding: '24px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px', background: '#0b0f19' }}>
                                {chatLog.map(m => (
                                    <div key={m.id} style={{ alignSelf: m.sender === 'user' ? 'flex-start' : 'flex-end', maxWidth: '70%' }}>
                                        <div style={{ background: m.sender === 'user' ? '#1e293b' : (m.sender === 'bot' ? '#10b981' : '#00f2fe'), color: m.sender === 'user' ? 'white' : (m.sender === 'bot' ? 'white' : 'black'), padding: '10px 16px', borderRadius: '12px', fontSize: '0.9rem' }}>
                            ...
                            <div style={{ padding: '20px', borderTop: '1px solid #1e293b', display: 'flex', gap: '12px' }}>
                                <input 
                                    ...
                                    disabled={!isManual}
                                    style={{ flex: 1, background: '#0b0f19', border: '1px solid #1e293b', borderRadius: '8px', padding: '12px', color: 'white' }}
                                />
                                <button 
                                    onClick={sendReply}
                                    disabled={!isManual || !replyText.trim()}
                                    style={{ padding: '0 24px', borderRadius: '8px', background: isManual ? '#00f2fe' : '#1e293b', color: 'black', border: 'none', fontWeight: 700, cursor: 'pointer' }}
                                >전송</button>
                            </div>
                        </>
                    ) : (
          ...
          {activeMenu === 'training' && (
            <div className="admin-panel" style={{ maxWidth: '1000px' }}>
                <div className="admin-panel-title">
                    <Cpu size={20} color="#00f2fe" /> AI CX/FAQ 학습 (Intelligence Training)
                </div>
                ...
                {/* 1. 빠른 누적 학습 (Append) */}
                <div className="admin-input-group" style={{ marginBottom: '32px', background: 'rgba(0, 242, 254, 0.05)', padding: '24px', borderRadius: '16px', border: '1px solid rgba(0, 242, 254, 0.2)' }}>
                    <label style={{ color: '#00f2fe', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <TrendingUp size={16} /> 빠른 지식 추가 (기존 매뉴얼 밑에 누적)
                    </label>
                    <textarea 
                        value={newRule}
                        onChange={e => setNewRule(e.target.value)}
                        style={{ width: '100%', height: '100px', backgroundColor: '#0b0f19', color: '#f8fafc', border: '1px solid #00f2fe', borderRadius: '12px', padding: '16px', fontSize: '0.95rem', fontFamily: 'inherit', resize: 'vertical', outline: 'none', marginBottom: '16px' }}
                    ...
                    <button 
                        ...
                        style={{ padding: '12px 24px', width: '100%', background: '#00f2fe', color: '#000', fontWeight: 800 }}
                    >
                ...
                <div className="admin-input-group">
                    ...
                    <textarea 
                        ...
                        style={{ width: '100%', height: '400px', backgroundColor: '#0b0f19', color: '#f8fafc', border: '1px solid #334155', borderRadius: '12px', padding: '20px', fontSize: '1rem', fontFamily: 'inherit', resize: 'vertical', outline: 'none' }}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '24px' }}>
                    ...
                    <button 
                        ...
                        disabled={isSaving}
                        style={{ padding: '14px 40px', background: '#1e293b', border: '1px solid #334155' }}
                    >
